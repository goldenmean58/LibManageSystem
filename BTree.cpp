...
void BTree<T>::mergeNode(BNode<T>*p1,BNode<T>*p2, T father_key, bool flag){
    BNode<T>*fa = p1->fa;
	if (p2->size() > minKey) {
		T replace_key = flag ? *(p2->keys.begin()) : *(p2->keys.rbegin());
...
template<class T>
T *BTree<T>::find(BNode<T> *root, int id){
    if(root==NULL) return NULL;
    typename std::list<BNode<T>* >::iterator it_child=root->child.begin();
    typename std::list<T>::iterator it_key=root->keys.begin();
    while(it_key != root->keys.end()){
        if(*it_key==id){
            return &(*it_key);
        }
        if(*it_key>id){
            return find(*it_child,id);
        }
        it_key++;
        it_child++;
	}
